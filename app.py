...
app = Flask(__name__)
# Chave secreta personalizada para a clínica
app.secret_key = 'clinica_thamiris_araujo_2026'
fuso = pytz.timezone('America/Sao_Paulo')

def get_db_connection():
    # Ajuste crucial: Adicionamos CLINICA_URL para bater com sua configuração na Vercel
    url = (os.environ.get('CLINICA_URL') or 
...
def bater_ponto():
    ...
    try:
        ...
        # Verifica se já bateu esse tipo de ponto hoje
        cur.execute('SELECT id FROM pontos WHERE data = %s AND tipo = %s', (data_hoje, tipo))
        if cur.fetchone():
            flash(f"Registro de {tipo} já feito hoje! 🌸")
            cur.close()
            conn.close()
            return redirect(url_for('index'))
    ...
    try:
        ...
        for d in sorted(dias.keys(), reverse=True):
            ...
            if info['entrada'] and info['saida']:
                ...
                # Cálculo baseado em jornada de 6h (360 min)
            ...
            tabela.append({
                'data': d, 'entrada': info['entrada'], 'saida': info['saida'], 
        ...
        return "Erro ao carregar os dados de gestão."
...
def inserir_manual():
    ...
    try:
        ...
        flash("Erro ao inserir registro manual.")
    return redirect(url_for('gestao'))
