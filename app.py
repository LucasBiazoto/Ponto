...
@app.route('/')
def index():
    conn = sqlite3.connect('ponto.db'); cursor = conn.cursor()
    cursor.execute("SELECT nome FROM colaboradoras ORDER BY nome ASC")
    colab = [row[0] for row in cursor.fetchall()]; conn.close()
...
def painel_gestao():
    ...
    for cid, n in colab_lista:
        cursor.execute("SELECT tipo, horario FROM registros WHERE nome = ? AND horario LIKE ?", (n, f"%{filtro}%"))
        regs = cursor.fetchall()
        if regs and regs[-1][0] == 'Entrada' and regs[-1][1].startswith(datetime.now().strftime("%d/%m/%Y")):
            presentes.append(n)
        datas = sorted(list(set([r[1].split(' ')[0] for r in regs])))
        total_saldo = 0.0; dias = 0
        for d in datas:
            e = [r[1] for r in regs if r[1].startswith(d) and r[0] == 'Entrada']
            s = [r[1] for r in regs if r[1].startswith(d) and r[0] == 'Saída']
            if e and s:
    ...
    ultimos = cursor.fetchall(); conn.close()
...
def lancar_manual():
    ...
    conn.commit(); conn.close(); return redirect(url_for('painel_gestao'))

@app.route('/exportar')
def exportar():
    conn = sqlite3.connect('ponto.db')
    df = pd.read_sql_query("SELECT nome, tipo, horario, localizacao FROM registros", conn)
    conn.close(); df.to_excel("Relatorio_Estetica.xlsx", index=False)
