import os
import psycopg2
import json  # Importação garantida no topo
...
def get_db_connection():
    # Prioriza a URL da Vercel Postgres
...
def gestao():
    ...
    try:
        ...
        return f"Erro ao carregar gestao: {str(e)}"
...
def inserir_manual():
    ...
    try:
        ...
        cur.execute('INSERT INTO pontos (tipo, data, mes, hora, geo) VALUES (%s, %s, %s, %s, %s)',
                    (request.form.get('tipo'), data_f, data_f.split('/')[1], request.form.get('hora'), "Manual (Gestora)"))
